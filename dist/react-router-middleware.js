...
module.exports = function (routes, options) {
  ...
  function renderApp(location, extraProps, cb) {
    ...
    router.run(function (Handler, state) {
      if (state.routes[0].name == "notfound") {
        var html = React.renderToStaticMarkup(React.createElement(Handler, null));
  ...
  function renderAppHandler(res, next, err, special, html, props, title) {
    ...
    if (!special) {
      return res.render("react", { title: title, html: html, props: props });
    }

    if (special.notFound) {
      res.status(404).render("react-404", { title: title, html: html });
    } else if (special.redirect) {
      res.redirect(303, special.redirect.to);
    } else if (special.render) {
      var url = special.render.url;
      var extraProps = special.render.extraProps;
